...
from .models import User, Movie, StreamSite
...
def login_required(view):
    ...
    def wrapped_view(**kwargs):
        if g.user is None:
            accessLogger.info('User attempted to access page whilst not logged in - returned to auth.login')
...
def register():        
        ...
        if form.validate_on_submit():
                ...
                if (emailExists(form.email.data)):
                    ...
                    db.session.add(u)
                    db.session.commit()
                    accessLogger.info(f'Registered (id:{u.userId}, username:{u.username})')
                    return redirect(url_for('auth.login'))
...
def login():
    ...
    if form.validate_on_submit():
        ...
        # Log user into session
            ...
            accessLogger.info(f'Login SUCCESS (id:{user.userId}, username:{user.username})')
